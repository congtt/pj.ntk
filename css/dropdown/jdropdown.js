...
(function($){
	...
	open = function(event){
		...
		if($.isFunction(data.renderMenu)){
			if($.isFunction(data.renderItem)){
				var ul = data.renderItem(data.renderMenu(), data.items,data.key,data.rel);
			}else{
				var ul = renderItem(data.renderMenu(), data.items,data.key,data.rel);
			}
		}else{
			if($.isFunction(data.renderItem)){
				var ul = data.renderItem($( '<ul></ul>' ), data.items,data.key,data.rel);
			}else{
				var ul = renderItem($( '<ul></ul>' ), data.items,data.key,data.rel);
			}
		}
	...
	renderItem = function(menu, items,key,rel){
		$.each(items, function(i, item){
			
			$('<li></li>').data('jdropdown.item', item).append(
				$( "<a></a>" ).attr({
					'href': '#', 'class': item['class'],'key':key,'rel':rel					
