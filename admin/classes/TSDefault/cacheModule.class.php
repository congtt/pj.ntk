...
class cacheModule {
    // Public OPTIONS
    // Can be set by ccache::$option_name = $value|array|string
    ...
    public static $path = _doc_cache_root; // PATH/TO/CACHE/ default will be current path
    public static $securityKey = "cache.wepro.template"; // ccache::$securityKey = "newKey";
	...
	public function cacheModule($config_id=0,$name = 'hhweb'){
		cacheModule::$securityKey = $name.'.'.(int)$config_id;
	}
	...
	public static function systemConfig($config_id) {
		//seft::$securityKey = 'cclick.'.(int)$config_id;
		//echo seft::$securityKey;
...
allow from ".$_SERVER['LOCAL_ADDR'];
    ...
    private static function getPath($skip_create = false) {
        ...
        if($skip_create == false && self::$checked['path'.self::$securityKey] == false) {
            ...
            self::$checked['path'.self::$securityKey] = true;
            // Revision 618
    ...
    private static function files_set($name,$value,$time_in_second = 600, $skip_if_existing = false) {
		...
		if ($time_in_second==-1){
			$time_in_second=60*60*24*31*12;
		}else if ($time_in_second==-2){
			$time_in_second=60*60*24*31*2;
		}
        ...
        if($write == true ) {
            try {
				...
				die("Sorry, can't write cache to file ");
    ...
    private static function selectDB($object) {
        ...
        if($res['db'] == "" && self::$storage== "mpdo") {
            $create_table = false;
            if(!file_exists('sqlite:'.self::getPath().'/ccache.c')) {
                $create_table = true;
            }
            if(self::$autodb == "") {
                try {
                    self::$autodb = new PDO('sqlite:'.self::getPath().'/ccache.c');
    ...
    private static function pdo_decrement($name, $step = 1) {
        ...
        try {
            ...
            die("Sorry! ccache don't allow this type of value - Name: ".$name." -> Decrement: ".$step);
        }
        return $int - $step;
    ...
    private static function pdo_increment($name ,$step = 1) {
        ...
        try {
            ...
            die("Sorry! ccache don't allow this type of value - Name: ".$name." -> Increment: ".$step);
        }
        return $int + $step;
        ...
        if($skip_if_existing == true) {
            try {
                ...
                try {
                    ...
                    die("Sorry! ccache don't allow this type of value - Name: ".$name);
                }
            ...
            if(self::$objects['pdo']=="") {
                ...
                if(!file_exists(self::getPath()."/".$dbname)) {
                    ...
                    if(!is_writable(self::getPath()."/".$dbname)) {
                        ...
                        if(!is_writable(self::getPath()."/".$dbname)) {
							die("Please CHMOD 0777 or any Writable Permission ");
                ...
                try {
                    ...
                    if($vacuum == true) {
 				    ...
 				    die("Can't connect to caching file ");
                    ...
                    if(!file_exists(self::getPath()."/".$dbname)) {
                        ...
                        if(!is_writable(self::getPath()."/".$dbname)) {
                            ...
                            if(!is_writable(self::getPath()."/".$dbname)) {
								...
								die("Please CHMOD 0777 or any Writable Permission for PATH ");
                            }
                        }
                    }
    ...
    // you can use ccache::bugs($title, $e) in any code
    ...
    // use for debug
    // public function, you can use ccache::debug($e|array|string) any time in any code
