...
abstract class DBManager
{
        ...
        )
    {
        ...
        $do_the_dump = isset($ts_config['dump_slow_queries']) 
            ? $ts_config['dump_slow_queries'] : false;
        $slow_query_time_msec = isset($ts_config['slow_query_time_msec']) 
            ? $ts_config['slow_query_time_msec'] : 5000;
    ...
    */
        ...
        )
    {
        ...
        $my_db_helper = 'MysqlHelper';
        if( $ts_config['dbconfig']['db_type'] == "mysql" ) {
            if (!isset($ts_config['mysqli_disabled']) or $ts_config['mysqli_disabled']==false) {
        ...
        if($db_type == "oci8" ){
            ...
            require_once('include/db/MssqlHelper.php');
            $my_db_helper = 'MssqlHelper';
        }
        if($my_db_helper == 'MysqlHelper'){
            require_once('include/db/MysqlHelper.php');
        }
        ...
        )
    {
		...
		if($count == -1){
			$count 	= $ts_config['list_max_entries_per_page'];
		}
