...
require_once('include/db/MysqlManager.php');
        ...
        )
    {
        ...
        if (mysqli_errno($this->getDatabase())){
            if($this->dieOnError || $dieOnError){
                ...
                ts_die ($msg."MySQL error ".mysqli_errno($this->database).": ".mysqli_error($this->database));
            }
            else{
        ...
        )
    {
        ...
        if(!isset($this->database)) {
        	...
        	$this->database = mysqli_connect($dbhost,$configOptions['db_user_name'],$configOptions['db_password'],$configOptions['db_name'],$dbport) 
                or ts_die("1Could not connect to server ".$dbhost." as ".$configOptions['db_user_name'].". port " .$dbport . ". " . mysqli_connect_error());
        }
        @mysqli_select_db($this->database,$configOptions['db_name']) 
            or ts_die( "Unable to select database: " . mysqli_connect_error());
