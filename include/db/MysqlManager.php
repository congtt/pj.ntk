...
class MysqlManager extends DBManager
{
	...
	{
		$mysqli = new mysqli($this->iHost, $this->iUser,$this->iPwd,$this->iDB);	
		...
		if ($result){
			...
			die('Error: #10 - 159 Lines - sp not found');
		}		
		return $error;	
	}
        ...
        )
    {
		...
		$this->checkError($msg.' SQL Failed:::', 'Syntax errors');
        if($autofree)
        ...
        )
    {
        ...
        if (!$this->database) {
            $this->database = mysql_connect(
                ...
                or ts_die("1Could not connect to server ".$configOptions['db_host_name']." as ".
        ...
        @mysql_select_db($configOptions['db_name']) 
            or ts_die( "Unable to select database: " . mysql_error($this->database));
